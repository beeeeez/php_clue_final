...
function waitingCheck()
{
    ...
    if ($stmt->execute($binds) > 0)
    {
        ...
        foreach ($results as $x)
        {
            ...
            else if ($x["TurnID"] == $_SESSION["PlayerID"] && $_SESSION['mode'] != "wait" && $_SESSION['mode'] != "move" && $_SESSION['mode'] != "suggest" && $_SESSION['mode'] != "accuse" && $_SESSION['mode'] != "dead")
            {
                $_SESSION["mode"] = "move";
            }
...
function movementCheck()
{
    ...
    if (isset($_POST['updateX']) && isset($_POST['updateY']))
    {
        whereami();
        $room = roomCheck($_SESSION['xCoord'], $_SESSION['yCoord']);
        if ($room != "none")
